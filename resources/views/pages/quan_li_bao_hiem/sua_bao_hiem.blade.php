...
@section('content')
    <div class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <form method="post" action="{{ route('update_bao_hiem', ['id' => $bao_hiem->ma_bh]) }}"
                        autocomplete="off" class="form-horizontal">
                        @csrf
                        @method('post')
                        <div class="card ">
                            <div class="card-header card-header-primary">
                                <h4 class="card-title">{{ __('Sửa bảo hiểm') }}</h4>
                            </div>
                            <div class="card-body ">
                                @if (session('status'))
                                    <div class="row">
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('so_bh') ? ' has-danger' : '' }}">
                                            <input class="form-control{{ $errors->has('so_bh') ? ' is-invalid' : '' }}"
                                                name="so_bh" value="{{ $bao_hiem->so_bh }}" id="input-so_bh" type="text"
                                                placeholder="{{ __('Sửa số bảo hiểm') }}" />
                                            @if ($errors->has('so_bh'))
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('ngay_cap') ? ' has-danger' : '' }}">
                                            <input
                                                ...
                                                name="ngay_cap" value="{{ $bao_hiem->ngay_cap }}" id="input-ngay_cap"
                                                type="date" placeholder="{{ __('Sửa ngày cấp') }}" required="true"
                                                aria-required="true" />
                                            @if ($errors->has('ngay_cấp'))
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('noi_cap') ? ' has-danger' : '' }}">
                                            <input class="form-control{{ $errors->has('noi_cap') ? ' is-invalid' : '' }}"
                                                name="noi_cap" value="{{ $bao_hiem->noi_cap }}" id="input-noi_cap"
                                                type="text" placeholder="{{ __('Sửa nơi cấp') }}" />
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('noi-kham_benh') ? ' has-danger' : '' }}">
                                            <input
                                                class="form-control{{ $errors->has('noi_kham_benh') ? ' is-invalid' : '' }}"
                                                name="noi_kham_benh" value="{{ $bao_hiem->noi_kham_benh }}"
                                                id="input-noi_kham_benh" type="text"
                                                placeholder="{{ __('Sửa nơi khám bệnh') }}" />
                                            @if ($errors->has('noi_kham_benh'))
                                                <span id=" noi_kham_benh-error" class="error text-danger"
                                        ...
                                        <select name="ma_nv" class="form-select" aria-label="Default select example">
                                            @foreach ($all_nhan_vien as $nhan_vien)
                                                <option value="{{ $nhan_vien->id }}"
                                                    {{ $bao_hiem->ma_nv === $nhan_vien->id ? 'selected' : '' }}>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-7">
                                        <select name="ma_loai_bh" class="form-select"
                                            aria-label="Default select example">
                                            @foreach ($all_loai_bh as $loai_bh)
                                                <option value="{{ $loai_bh->ma_loai_bh }}"
                                                    {{ $bao_hiem->ma_loai_bh === $loai_bh->ma_loai_bh ? 'selected' : '' }}>
                                                    {{ $loai_bh->ten_loai_bh }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
