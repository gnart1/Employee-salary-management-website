...
<head>
    ...
    <meta name="csrf-token" content="{{ csrf_token() }}" />
    <title>{{ __('QUẢN LÝ NHÂN SỰ') }}</title>
...
<body class="{{ $class ?? '' }}">
    ...
    <script type="text/javascript">
        const session_success =
            "{{ strlen(Session::get('notify_success')) > 0 ? Session::get('notify_success') : 'false' }}";
        const session_danger =
            "{{ strlen(Session::get('notify_danger')) > 0 ? Session::get('notify_danger') : 'false' }}";

        if (session_success !== 'false') {
            md.showNotification('top', 'right', 'success', session_success);
        }

        if (session_danger !== 'false') {
            md.showNotification('top', 'right', 'danger', session_danger);
        }
    </script>
</body>
...
@php
Session::put('notify_success', null);
Session::put('notify_danger', null);
@endphp
